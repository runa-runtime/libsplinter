...
On **Debian / Ubuntu** (and derivatives), the Big Bang installer builds and installs the full semantic stack (Splinter with embeddings, llama.cpp, and Lua) from source. This will install all OS package dependencies needed to build splinter and its dependencies from source (takes ~15 minutes on average systems):
...
- **Pub/sub notifications** — 64 signal groups you can poll, plus an `eventfd`-backed event bus with a dirty mask so watchers wake only on what actually changed.
...
- **Block on the event bus (kernel-assisted).** The owner process arms an `eventfd` once with `splinter_event_bus_init()`. Any process then calls `splinter_event_bus_open()` and blocks in `splinter_event_bus_wait(fd, timeout_ms)` until a write advances the global epoch. On wake, `splinter_event_bus_get_dirty()` hands you a bitmask of which slots changed, so you rescan only what moved instead of sweeping the whole store. The mask is a fixed 1024 bits wide to stay cache-resident, so a store larger than that maps slots onto it in contiguous stripes of `splinter_event_bus_stripe()` slots per bit — a set bit is a short sequential run to rescan, and at 1024 slots or fewer it is exact. If you are the only consumer, prefer `splinter_event_bus_take_dirty()`, which snapshots and clears in one step so the mask cannot saturate. The fd is a normal pollable descriptor, so it drops cleanly into an existing `poll`/`epoll` reactor alongside your other I/O.
